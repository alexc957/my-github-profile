...
export default function Pagination({page, setPage, currentLength}) {
    return (
        <div className="row-space-between">     
            <button className="pag-link" onClick={()=> setPage(page - 1)}>{page>1 && <img width={12} height={12} src={PREVOIUS_ICON_URL}/>}</button> 
            {page} 
            <button  className="pag-link" onClick={()=>setPage(page + 1)} >{currentLength>0 && <img width={12} height={12} src={NEXT_ICON_URL} />}</button>
        </div>
    )
}
